    ...
    $.fn.sidebarMenu = function (options) {
        ...
        function init(target, data) {
            $.each(data, function (i, item) {
                var li = $('<li id="li_'+item.menuId+'"></li>');
                var a = $('<a></a>');
                var icon = $('<i></i>');
                //icon.addClass('glyphicon');
                icon.addClass(item.menuIcon);
                var text = $('<span></span>');
                ...
                if (item.menus&&item.menus.length>0) {
                    ...
                    var href = 'javascript:addTabs({id:\'' + item.menuId + '\',title: \'' + item.menuName + '\',close: true,url: \'' + item.menuUrl + '\',icon:\''+ item.menuIcon +'\'});';
                    //var href = 'javascript:addTabs({url: \'' + item.url +  '\'});';
                    a.attr('href', href);
                    //if (item.istab)
                    //    a.attr('href', href);
                    //else {
